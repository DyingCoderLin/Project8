...
const TableSelectionModal = ({ navigation,isVisible, onClose, onSelect,defaultTable }) => {
  ...
  const [tableNameAndID, setTableNameAndID] = useState([]);
    ...
    useEffect(() => {
        axios({
            method: 'get',
            url: 'http://192.168.173.144:8080/getAllTableInfo',
            ...
            if (response.data.code /*&& response.data.tableArray*/) {
                ...
                console.log(response.data.tableArray);
                if (response.data.tableArray) {
                    const tableNames = response.data.tableArray.map(table => table.tableName);
                    setTable(tableNames);
                    setTableNameAndID(response.data.tableArray);
                }
            } else {
  ...
  const handleDone = () => {
    ...
    axios({
      method: 'post',
      url: 'http://192.168.173.144:8080/switchTable',
